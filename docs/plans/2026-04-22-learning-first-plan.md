...
| `tests/pressure-scenarios.md` | Subagent pressure test scenarios for quality benchmark |
...
This is the most important file — it defines all agent behavior. It follows ALL superpowers patterns: frontmatter with triggering-conditions-only description, HARD-GATE, SUBAGENT-STOP, Iron Law, Red Flags table, Rationalization table, persuasion principles, Socratic questioning patterns, graphviz decision diagrams, announcement pattern, and consistent voice.
...
git commit -m "feat: add core learning-first skill with superpowers patterns

Includes: HARD-GATE, SUBAGENT-STOP, Iron Law, Red Flags table,
Rationalization table, Socratic method, persuasion principles,
announcement pattern, consistent voice, graphviz flow diagram."
```
...
### Task 14: Quality Benchmark — Pressure Testing

**Files:**
- Create: `tests/pressure-scenarios.md`

This follows superpowers' TDD-for-skills methodology: RED (baseline failure without skill) → GREEN (compliance with skill) → REFACTOR (close loopholes).
...
```bash
git add tests/pressure-scenarios.md
git commit -m "test: add pressure test scenarios for skill quality benchmark

4 scenarios testing: time pressure, authority/confidence,
simplicity bias, exhaustion/sunk cost. Follows superpowers'
TDD-for-skills RED/GREEN/REFACTOR methodology."
```
